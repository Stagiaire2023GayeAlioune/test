...
def main():
    ...
    if uploaded_files is not None:
            ...
            if col5.button("Méthode mono_exponentielle"):
                st.latex(r''' f_decay(x,a,tau) =  \epsilon + a\exp (\frac{-x}{tau} ) ''')
            ...
            if col6.button("Méthode double_exponentielle"):
                st.latex(r''' f_decay(x,a1,t1,a2,t2) =  \epsilon + a1\exp (\frac{-x}{t1} ) +a2\exp (\frac{-x}{t2})''')
                st.latex(r''' Aire = a1t1 + a2t2 ''')
                for uploaded_file in uploaded_files:
                       df = pd.read_csv(uploaded_file, delimiter="\t")
            ...
            if col7.button("Méthode gaussienne "):
                st.latex(r'''f_decay (x,a1,t1,c) =  \epsilon + a1\exp (\frac{-x}{t1} )  +\frac{a2}{2}\exp (\frac{-x}{t1+1.177c} ) +\frac{a2}{2}\exp (\frac{-x}{t1-1.177c})''')
                for uploaded_file in uploaded_files:
                       df = pd.read_csv(uploaded_file, delimiter="\t")
